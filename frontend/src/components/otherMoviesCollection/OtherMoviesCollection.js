...
import style from "./otherMoviesCollection.module.scss"

import BackBtn from '../backBtn/BackBtn'
import DoubleArrowIcon from '@mui/icons-material/DoubleArrow';
import MovieCard from '../movieCard/MovieCard'
import HashLoader from "react-spinners/HashLoader";
export default function OtherMoviesCollection() {
  const [movie, setMovie] = useState(null)
  let [loading, setLoading] = useState(true);
  const getOtherMovies = async () => {
    try {

      const result = await fetch(("/movie/otherLanguage"), {
        method: "GET",
        headers: {
  ...
  return (
    <div className={style.other + " col-12 col-lg-9 sticky-top   border p-3"}>
      {
          loading &&
      ...
      <div className={style.mainContent + " row col-12 mx-auto  p-0"}>
        <div className={style.movies + ' row col-12  mx-auto py-3 p-0 my-2 '}>
          <div className={style.metadata + " col-12 d-flex  justify-content-between"}>
            <span className='my-auto'><DoubleArrowIcon /> Other Language Movies</span>
          </div>
